...
mod tests {
    ...
    // Benchmarks (only with nightly)

    #[bench]
    fn bench_encode_8_12_4(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![0x00, 0x01, 0x02, 0x03];
            let pt = vec![0x00, 0x01];
            let _ = encode::<u8, 26>(key, pt);
        });
    }

    #[bench]
    fn bench_decode_8_12_4(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![0x00, 0x01, 0x02, 0x03];
            let ct = vec![0x21, 0x2A];
            let _ = decode::<u8, 26>(key, ct);
        });
    }
    ...
    fn bench_encode_kernel_8_12_4(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![0x00, 0x01, 0x02, 0x03];
            let _ = encode_kernel::<u8, 26>(key, [0x00, 0x01]);
        });
    }
    ...
    fn bench_decode_kernel_8_12_4(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![0x00, 0x01, 0x02, 0x03];
            let _ = decode_kernel::<u8, 26>(key, [0x21, 0x2A]);
        });
    }

    #[bench]
    fn bench_encode_16_16_8(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07];
            let pt = vec![0x00, 0x01, 0x02, 0x03];
            let _ = encode::<u16, 34>(key, pt);
        });
    }

    #[bench]
    fn bench_decode_16_16_8(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07];
            let ct = vec![0x23, 0xA8, 0xD7, 0x2E];
            let _ = decode::<u16, 34>(key, ct);
        });
    }
    ...
    fn bench_encode_kernel_16_16_8(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07];
            let _ = encode_kernel::<u16, 34>(key, [0x0100, 0x0302]);
        });
    }
    ...
    fn bench_decode_kernel_16_16_8(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07];
            let _ = decode_kernel::<u16, 34>(key, [0xA823, 0x2ED7]);
        });
    }

    #[bench]
    fn bench_encode_32_20_16(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F,
            ...
            let pt = vec![0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07];
            let _ = encode::<u32, 42>(key, pt);
        });
    }

    #[bench]
    fn bench_decode_32_20_16(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F,
    ...
    fn bench_encode_kernel_32_20_16(b: &mut Bencher) {
        b.iter(|| {
            ...
            let _ = encode_kernel::<u32, 42>(key, [0x03020100, 0x07060504]);
        });
    }
    ...
    fn bench_decode_kernel_32_20_16(b: &mut Bencher) {
        b.iter(|| {
            ...
            let _ = decode_kernel::<u32, 42>(key, [0x0EDC0E2A, 0x73FF3194]);
        });
    }

    #[bench]
    fn bench_encode_64_24_24(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
            ...
            let _ = encode::<u64, 50>(key, pt);
        });
    }

    #[bench]
    fn bench_decode_64_24_24(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
            ...
            let _ = decode::<u64, 50>(key, ct);
        });
    }

    #[bench]
    fn bench_encode_kernel_64_24_24(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
            ];
            let _ = encode_kernel::<u64, 50>(key, [0x0706050403020100, 0x0F0E0D0C0B0A0908]);
        });
    }

    #[bench]
    fn bench_decode_kernel_64_24_24(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
            ];
            let _ = decode_kernel::<u64, 50>(key, [0x02CEDB0E827267A4, 0xDA7871AE32EAAB35]);
        });
    }

    #[bench]
    fn bench_encode_128_28_32(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B,
            ...
            let _ = encode::<u128, 58>(key, pt);
        });
    }

    #[bench]
    fn bench_decode_128_28_32(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B,
            ...
            let _ = decode::<u128, 58>(key, ct);
        });
    }

    #[bench]
    fn bench_encode_kernel_128_28_32(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B,
    ...
    #[bench]
    fn bench_decode_kernel_128_28_32(b: &mut Bencher) {
        b.iter(|| {
            let key = vec![
                0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
                0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B,
